...
% Geometric correction factors from the hydrodynamic calculation.
% beta modifies translational force balance; gamma modifies rotational mobility.
beta_ant = 0.13;
beta_mid = 0.30;
gamma_ant = 0.54;
gamma_mid = 0.54;
beta = mean([beta_ant,beta_mid]);
gamma = mean([gamma_ant,gamma_mid]);

F = U*6*pi*mu*a/beta; % raw force scale before beta correction
...
pos_20_03 = straight_meta_fn(U,0,20,20,a,...
    a,mu,dt,21,21,cilia_density,T,beta_mid,beta_ant,gamma_mid,gamma_ant);
...
pos_20_005 = straight_meta_fn(U,0,20,20,a,...
    a,mu,dt,125,125,cilia_density,T,beta_mid,beta_ant,gamma_mid,gamma_ant);
...
pos = straight_meta_fn(10,0,10,10,1,...
    1,1,dt,0.1,0.1,cilia_density,T,beta,beta,gamma,gamma);

plot(pos(:,1)+10,pos(:,3),'LineWidth',5)

%large omega, large k
pos = straight_meta_fn(10,0,10,10,1,...
    1,1,dt,1.9*pi,1.9*pi,cilia_density,T,beta,beta,gamma,gamma);

plot(pos(:,1)+10,pos(:,3)+12,'LineWidth',5)

%small omega, large k
pos = straight_meta_fn(10,0,0.1,0.1,1,...
    1,1,dt,1.9*pi,1.9*pi,cilia_density,1,beta,beta,gamma,gamma);
...
for ii = 1:length(omega_list) %loop over omega
    ...
    A = gamma*F*R0_list(jj)./(8*pi*mu*a^3);
...
for ii = 1:loop_omega %loop over omega
    for jj =1:loop_N %loop over lambda
        ...
        R0 = norm(R0);
    A = gamma*F*R0./(8*pi*mu*a^3*omega_temp);
...
for ii = 1:length(reversal_time_list)
    ...
    for jj = 1:n
        ...
        [pos,p_hat_list] =  reversal_w_spin_lag_cont_fn(U,omega_cell,omega_mid,omega_ant,reversal_time,lag_time,a_mid,a_ant,mu,start_time,dt,lambda_mid,lambda_ant,0,beta_mid,beta_ant,gamma_mid,gamma_ant);
...
for jj = 1:n
        ...
        [~,p_hat_list] =  reversal_w_spin_lag_cont_fn(U,omega_cell,omega1,omega2,reversal_time,0,a_mid,a_ant,mu,start_time,dt,lambda_mid,lambda_ant,0,beta_mid,beta_ant,gamma_mid,gamma_ant);
        idx1 = round(start_time/dt);
        ...
        [~,p_hat_list] =  reversal_w_spin_desync_fn(U,omega_cell,omega1,omega2,reversal_time,a_mid,a_ant,mu,start_time,dt,lambda_mid,lambda_ant,0,beta_mid,beta_ant,gamma_mid,gamma_ant);
        pz = p_hat_list(:,3);
        theta_rad = acos(pz);

        theta_list_desync (jj) = mean(rad2deg(theta_rad(idx2:end)));

        [~,p_hat_list] =  reversal_w_spin_step_fn(U,omega_cell,omega1,omega2,reversal_time,a_mid,a_ant,mu,start_time,dt,lambda_mid,lambda_ant,beta_mid,beta_ant,gamma_mid,gamma_ant);
        pz = p_hat_list(:,3);
        theta_rad = acos(pz);
...
for ii = 1:n_pts
    ...
    [~,r,p] = straight_meta_fn_sp(U,omega,a,mu,dt,R0,T,beta,gamma);
...
figure
pos = straight_meta_fn_sp(U,omega_list(1),a,mu,dt,chi_list(1),5,beta,gamma);
...
figure
pos = straight_meta_fn_sp(U,omega_list(2),a,mu,dt,chi_list(2),5,beta,gamma);
track1(1:length(pos),:) = pos;
...
figure
pos = straight_meta_fn_sp(U,omega_list(3),a,mu,dt,chi_list(3),5,beta,gamma);
track1(1:length(pos),:) = pos;
