...
if nargin < 14 || isempty(beta_mid),  beta_mid  = 1; end
if nargin < 15 || isempty(beta_ant),  beta_ant  = beta_mid; end
if nargin < 16 || isempty(gamma_mid), gamma_mid = 1; end
if nargin < 17 || isempty(gamma_ant), gamma_ant = gamma_mid; end
...
for ii = 1:N_step+1
    ...
    if ii>=idx1 && ii<=idx2
        ...
        counter_mid = max(0,ceil((t-start_time)/(reversal_time/n_mid)));
        mask_mid = logical([ones(counter_mid,1);zeros(n_mid-counter_mid,1)]);
        counter_ant = max(0,ceil((t-start_time)/(reversal_time/n_ant)));
    ...
    % 
    f_mag_mid_temp = F.*(sin((2*pi*a_mid/lambda_mid).*(angles_mid-omega_mw_mid*t))+1)./(n_mid+n_ant);
    f_mag_ant_temp = F.*(sin((2*pi*a_ant/lambda_ant).*(angles_ant-omega_mw_ant*t))+1)./(n_mid+n_ant);

    f_mag_mid(mask_mid) = -abs(f_mag_mid_temp(mask_mid));
    f_mag_ant(mask_ant) = -abs(f_mag_ant_temp(mask_ant));
    ...
    if ii>idx2
            f_mag_mid = -F.*(sin((2*pi*a_mid/lambda_mid).*(angles_mid-omega_mw_mid*t))+1)./(n_mid+n_ant);
            f_mag_ant = -F.*(sin((2*pi*a_ant/lambda_ant).*(angles_ant-omega_mw_ant*t))+1)./(n_mid+n_ant);
            omega_mw_mid(:) =  -omega_mw_mid_mag';
            omega_mw_ant(:) = -omega_mw_ant_mag';
    end 
    t = t+ dt;
...
make_my_video(delta_vec_list_mid, delta_vec_list_ant, ...
                       f_mag_list_mid, f_mag_list_ant,'desync.mp4')
end
end
